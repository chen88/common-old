...
gulp.task('serve:auth', serveTasks, function () {
  browserSyncInit([
...
gulp.task('auth', () => {
  gulp.start('serve:noUpdate:auth');
});

gulp.task('serve:noUpdate:auth', ['copyJade', 'copyCommonClasses', 'watch:noUpdate'], function () {
  browserSyncInit([
    path.join(conf.paths.app.tmp, '/serve'),
    conf.paths.app.src,
    conf.paths.common.root,
...
gulp.task('serve:dist:auth', ['build:noUpdate'], function () {
  browserSyncInit(conf.paths.app.dist, undefined);
});

gulp.task('serve:prod:auth', ['buildProd'], function () {
  browserSyncInit(conf.paths.app.dist, undefined);
});

gulp.task('serve:prodSimple:auth', [], function () {
  browserSyncInit(conf.paths.app.dist, undefined);
});

gulp.task('simpleServe:auth', function () {
  browserSyncInit([
    path.join(conf.paths.app.tmp, '/serve'),
    conf.paths.app.src,
    conf.paths.common.root,
